# -*- coding: utf-8 -*-
"""Render plotly figures inside a QtWebKit ``QWebView``.
...
``fig.to_html(include_plotlyjs="cdn")`` renders blank. Mirroring the qgis-AGLgis
plugin, we instead:

  1. vendor the last v1 plotly.js (``assets/plotly-1.58.5.min.js``) and inline it,
...
The same ``render_html`` feeds both the in-plugin view and "open in browser", so
the chart is identical in both.
"""
import base64
import json
import os
...
def show_in_webview(web_view, fig, config, previous_path=None):
    ...
    fd, path = tempfile.mkstemp(suffix=".html", prefix="climaplots_")
...
def open_in_browser(fig, config):
    ...
    html = render_html(fig, config)
    fd, path = tempfile.mkstemp(suffix=".html", prefix="climaplots_browser_")
    with os.fdopen(fd, "w", encoding="utf-8") as f:
        f.write(html)
