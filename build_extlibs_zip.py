...
ClimaPlots' scientific stack (xarray, pymannkendall, pyhomogeneity, climdex and
their compiled deps such as bottleneck) is ABI-locked to the Python version, so
a single bundle breaks when QGIS ships a different Python. This builds one zip
per interpreter, tagged ``extlibs-<cpXY>-<platform>.zip``, so the runtime
downloader (extlibs_manager) can fetch the build matching each QGIS Python.

numpy / pandas / scipy / requests / plotly come WITH QGIS and must never be
shadowed from extlibs/, so they are stripped from the build.
...
Two modes:
  ...
  python build_extlibs_zip.py
      ...
      packages, applies the climdex pandas-3 patch ('1M' -> 'ME'), and writes
      extlibs-<tag>.zip (e.g. extlibs-cp312-win_amd64.zip) to the plugin root.

  # Manual mode — zip an existing pip --target dir:
  python build_extlibs_zip.py <build_dir> <out_zip>

Commit + push the resulting extlibs-<tag>.zip so the runtime downloader can
fetch the build matching each QGIS Python. The GitHub Actions workflow
(.github/workflows/build-extlibs.yml) builds the full matrix.
"""
import os
import shutil
import subprocess
...
def full_build():
    ...
    build = tempfile.mkdtemp(prefix="climaplots_extlibs_")
    try:
